...
def predict():
    ...
    map = folium.Map(location=[22.5726, 88.3639], zoom_start=4)
    # Add markers to the map for each city
    for city, prediction_data in predictions.items():
        prediction = prediction_data['prediction']
        lat = prediction_data['lat']
        lon = prediction_data['lon']
        # Add a popup to the marker with the prediction value
        popup_text = "{}: {:.2f}".format(city, prediction)
        folium.Marker(location=[lat, lon], popup=popup_text).add_to(map)

    # Save the map as an HTML file and return it
